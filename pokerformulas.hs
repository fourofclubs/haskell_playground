...
main = do 
    ...
    if M.null pd then putStrLn "No deal found."
    else do
        putStrLn "Players?"
        mapM_ print (keys pd)
...
data PredicateType = IsValue Value | IsSuit Suit | IsNotValue Value | ValueAtMost Value | IsCard Value Suit | IsAnything 
    | And PredicateType PredicateType | Not PredicateType deriving (Show, Eq)
...
straightValues v = L.take 5 [v, pred v ..] ++ (if v <= FIVE then [ACE] else [])
...
cardsFor (StraightFlush v s) = L.map (flip IsCard s) (straightValues v)
...
deal p cp d = let (top, bottom) = L.splitAt p d in
    fmap (\a -> (a,bottom)) . fmap ((`mod` p) . (+1)) $ (findLastIndex cp top)

--second deal to dealer
deal' :: Players -> (Card -> Bool) -> Deck -> Maybe (Int, Deck)
deal' p cp d = let (top, bottom) = L.splitAt p d in if L.null bottom then Nothing else 
    if cp (head bottom) then Just (p, (last top : tail bottom)) else Nothing

--bottom deal
deal'' :: Players -> (Card -> Bool) -> Deck -> Maybe (Int, Deck)
deal'' p cp d = let (top, bottom) = L.splitAt p d in if L.null bottom then Nothing else
    if cp (last bottom) then Just ((-1), (last top : init bottom)) else Nothing
...
search d p h = 
    let cards = (L.map . L.map $ toF) $ (nub . permutations . cardsFor $ h)
        deals = catMaybes . L.map (search' d p) $ cards
    in bestDeal p deals
...
selectAll hb = L.map (fmap (selectOption hb)) (flip zip [1..] (optionList hb))
